...
class OBJECT_OT_export_check_dialog(bpy.types.Operator):
    bl_idname = "object.export_check_dialog"
    bl_label = "Export Check Dialog"
    bl_options = {'REGISTER'}
...
class OBJECT_OT_do_export(bpy.types.Operator):
    bl_idname = "object.do_export"
    bl_label = "Export"
    bl_options = {'REGISTER', 'INTERNAL'}

    def execute(self, context):
        global _EXPORT_SETTINGS, _EXPORT_CONTEXT, _VALIDATION_DATA
        if _EXPORT_SETTINGS and _EXPORT_CONTEXT:
            success, message = do_export(_EXPORT_SETTINGS, _EXPORT_CONTEXT)
            if success:
                self.report({'INFO'}, message)
...
class OBJECT_OT_cancel_export_check(bpy.types.Operator):
    bl_idname = "object.cancel_export_check"
    bl_label = "Cancel"
    bl_options = {'REGISTER', 'INTERNAL'}
...
class OBJECT_OT_select_export_path(bpy.types.Operator):
    ...
    def execute(self, context):
        context.scene.export_to_ue_settings.export_path = self.filepath
